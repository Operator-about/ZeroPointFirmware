...
uint32_t Buffer[32768] __attribute__((section(".kernel_buffer")));

void exFAT_init(){
    debug("[^]MBR init stage\r\n");

    read_block(0);
    if(DAT_buffer[511] != 0x0){
        debug("[+]Zero sector read done\r\n");
    }
    ...
    _MBR = (MBR*)DAT_buffer;
    if(_MBR->Signature[0] == 0x55 || _MBR->Signature[1] == 0xAA){
        debug("[+]Data reciving\r\n");
    }
    ...
    if(_MBR->PartionRecord[1].Start_LBA != 0x0){
        for(int _build_LBA = 0; _build_LBA < 4; _build_LBA++){
            _LBA |= ((uint32_t)_MBR->PartionRecord[1].Start_LBA[_build_LBA] << (8 * _build_LBA));
        }
        debug("[+]LBA reciving\r\n");
    }
    ...
    Buffer[0] = 0x0;
    read_cluster(exFAT_attr.LBA_data_regione + ((exFAT_attr.FirstClusterRoot - 2) * exFAT_attr.SectorsPerCluster),Buffer);
    KernelFile _kernel = get_first_cluster_kernel(Buffer);
    ...
    if(_kernel.NoFATChain & (1ULL << 1)){
        for(int _get = 0; _get < (int)(_kernel.DataLength / (exFAT_attr.BytsPerSector * exFAT_attr.SectorsPerCluster)); _get++){
            read_cluster(exFAT_attr.LBA_data_regione + ((_kernel.FirstCluster - 2) * exFAT_attr.SectorsPerCluster),Buffer);
            debug("[^]Current cluster get\r\n");
        }
...
void read_cluster(uint32_t _cluster, uint8_t Buffer[]){
    int _out_index = 0;
    ...
    for(int _current_sector = 0; _current_sector < exFAT_attr.SectorsPerCluster; _current_sector++){
        ...
        for(int _current_byte = 0; _current_byte < exFAT_attr.BytsPerSector; _current_byte++){
           Buffer[_out_index] = DAT_buffer[_current_byte];
            _out_index++;
        }
        _cluster++;
    }
...
KernelFile get_first_cluster_kernel(uint8_t Buffer[]){
    //Поиск файла kernel8.img
    uint8_t _tempory_buffer[32]; //Временный буфер под дескрипторов
    ...
    debug("[^]Search ZeroPoint.img\r\n");
    debug("[^]File or folders:\r\n");
    for(int _index = 0; _index < (int)(exFAT_attr.BytsPerSector * exFAT_attr.SectorsPerCluster); _index++){
        ...
        if(_tempory_buffer_index == 32){
            //Проверка на файл
            if(_tempory_buffer[0] == 0x85){
                _secondary_index = 0;
                _name_count = 0;
                _kernel.EntryType = _tempory_buffer[0];
                _kernel.SecondaryCount = _tempory_buffer[1];
                _kernel.FileAttributes[0] = _tempory_buffer[4];
                _kernel.FileAttributes[1] = _tempory_buffer[5];
                _secondary_count = (int)_kernel.SecondaryCount;
            }

            if(_secondary_index < _secondary_count && _tempory_buffer[0] == 0xC0){
                _kernel_stream.EntryType = _tempory_buffer[0];
                _kernel_stream.GeneralSecondaryFlags = _tempory_buffer[1];
                _kernel_stream.NameLength = _tempory_buffer[3];
                for(int _build_index = 0; _build_index < 4; _build_index++){
                    _kernel_stream.FirstCluster |= ((uint32_t)_tempory_buffer[20 + _build_index] << (8 * _build_index));
                }

                for(int _build_length = 0; _build_length < 8; _build_length++){
                    _kernel_stream.DataLength |= ((uint64_t)_tempory_buffer[24 + _build_length] << (8 * _build_length));
                }
                _secondary_index++;
            }

            if(_secondary_index < _secondary_count && _tempory_buffer[0] == 0xC1){
                _kernel_name[_name_count].EntryType = _tempory_buffer[0];
                _kernel_name[_name_count].GeneralSecondaryFlags = _tempory_buffer[1];
                for(int i = 0; i < 30; i++){
            ...
            if(_secondary_index >= _secondary_count){
                int _jump_index = 0;
                while(_name_index < _name_count){
                    for(int j = 0; j < 30; j++){
                        UARTPL011M.UARTPL011_REGISTERS->UART_DR = _kernel_name[_name_index].FileName[j];
                ...
                debug("\r\n");
                if(compare_ZeroPoint(_name, "ZeroPoint.img") == 1){
                    if(_kernel_stream.GeneralSecondaryFlags & (1ULL << 1)){
                        debug("[^]NoFATChain\r\n");
                    }

                    debug("[+]This ZeroPoint.img\r\n");
...
uint32_t walk_FAT_table(uint32_t _cluster){
    ...
    if(_tempory_cluster >= 0xFFFFFFF8 && _tempory_cluster <= 0xFFFFFFFF){
        debug("[^]Cluster non-have continues\r\n");
    }
    else if(_tempory_cluster == 0x0){
        debug("[^]Free cluster or NoFATChain\r\n");
    }
    else{
        debug("[^]Cluster have continues\r\n");
